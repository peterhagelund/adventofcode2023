...
func main() {
	...
	for x := 0; x < len(contraption[0]); x++ {
		tileCount = max(tileCount, calculateEnergizedTiles(contraption, leg{0, x, down}))
		tileCount = max(tileCount, calculateEnergizedTiles(contraption, leg{len(contraption) - 1, x, up}))
	}
	for y := 0; y < len(contraption); y++ {
		tileCount = max(tileCount, calculateEnergizedTiles(contraption, leg{y, 0, right}))
		tileCount = max(tileCount, calculateEnergizedTiles(contraption, leg{y, len(contraption[0]) - 1, left}))
	}
